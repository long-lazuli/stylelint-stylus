...
async function verifyCore(expectation, options, root, result) {
    const verify = (await coreRule)(expectation, options, {
...
function rule(expectation, options) {
    return async (root, result) => {
        if (root.source.lang !== "stylus") {
            await verifyCore(expectation, options, root, result)
            return
        }
            ...
            parseSelector(rawSelector, result, ruleNode, (selectorTree) => {
                selectorTree.walkTags((tagNode) => {
                    ...
                    report({
                        ...
                        endIndex: tagNode.sourceIndex + tagName.length,
                        ruleName,
                        result,
                    })
                })
